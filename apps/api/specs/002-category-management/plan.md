...
Work 연결 삭제 제한은 category application delete flow에서 work application port를 통해 처리한다.
...
**Constraints**: category는 work infrastructure/domain 구현체를 직접 import하지 않는다. Work 참조 확인이 필요하면 work application port만 사용한다.
...
| domain이 presentation/infrastructure에 의존 안 함 | ✅ | `Category`, `CategoryRepository`는 순수 domain |
